...
$quoteUrl = $whatsappBase . '?text=' . rawurlencode('Hello Royal Bali, I would like to request a linen rental quotation.');
$products = [
...
<head>
    ...
    <meta name="description" content="Royal Bali Laundry Rentals offers towels, pillowcases, sheets and duvets for hotels. Explore our linen collection and request a rental quotation.">
    <meta name="theme-color" content="#092849">
    <title>Royal Bali Laundry Rentals — Linen Rental for Hotels</title>
...
<header class="sticky top-0 z-40 border-b border-slate-100 bg-white/95 backdrop-blur">
    <div class="wrap flex h-20 items-center justify-between gap-6">
        <a href="#beranda" aria-label="Royal Bali Laundry Rentals — home" class="flex items-center gap-3">
            ...
            <span><span class="block font-display text-2xl tracking-wide">ROYAL BALI</span><span class="block text-center text-[9px] tracking-[.43em]">LAUNDRY RENTALS</span></span>
        </a>
...
<main id="main">
    <section id="beranda" class="relative isolate overflow-hidden bg-navy text-white">
        ...
        <div class="wrap py-20 md:py-24 lg:py-28">
            <div class="max-w-2xl">
                <p class="eyebrow !text-[#e3b873]">ROYAL BALI · LINEN RENTAL</p>
        ...
        <div class="wrap">
            ...
            <?php foreach ($products as $index => $product): ?>
                <article class="flex flex-col overflow-hidden rounded-sm border border-slate-200 bg-white">
                    ...
                    <a href="<?= e($whatsappBase . '?text=' . rawurlencode('Hello Royal Bali, I would like to enquire about ' . $product['name'] . '. Please share availability and a rental quotation.')) ?>" target="_blank" rel="noopener noreferrer" class="btn btn-primary mt-auto !px-3 !text-xs">Enquire About This Collection <?= icon('arrow', 'h-4 w-4') ?></a></div>
                </article>
            <?php endforeach; ?>
            </div>
...
<footer id="kontak" class="bg-cream py-12"><div class="wrap"><div class="grid gap-8 md:grid-cols-[1fr_1fr_auto]"><div><p class="font-display text-2xl">ROYAL BALI</p><p class="mt-2 text-[10px] tracking-[.3em]">LAUNDRY RENTALS</p><p class="mt-4 max-w-xs text-sm leading-6 text-slate-600">Linen and towels to support the everyday needs of your hotel.</p></div><div><h2 class="text-sm font-semibold">Visit & Contact Us</h2><address class="mt-4 text-sm not-italic leading-7 text-slate-600">Jalan Gunung Patas 1 No. 81B<br>Padang Sambian Kelod, Denpasar Barat<br><a class="hover:text-blue" href="tel:+6282237963375"><?= e($phone) ?></a></address></div><div class="flex flex-col gap-3 text-sm"><a href="#koleksi">Linen Collection</a><a href="#cara-sewa">How to Rent</a><a href="#faq">Rental Terms</a><a href="<?= e($quoteUrl) ?>" target="_blank" rel="noopener noreferrer">Request a Quote ↗</a></div></div><div class="mt-9 flex flex-wrap justify-between gap-3 border-t border-slate-200 pt-6 text-xs text-slate-500"><p>© <?= date('Y') ?> Royal Bali Laundry Rentals.</p><p>Illustrative images, not photographs of actual products.</p></div></div></footer>
<script>
...
form.addEventListener('submit', event => { event.preventDefault(); const data = new FormData(form); const date = new Date(data.get('date') + 'T12:00:00').toLocaleDateString('en-GB', {day:'numeric', month:'long', year:'numeric'}); const message = `Hello Royal Bali, I would like to request a linen rental quotation.\n\nName: ${data.get('name').trim()}\nProperty: ${data.get('property').trim()}\nCollection: ${data.get('product')}\nRental start date: ${date}\nRequirements: ${data.get('details').trim()}`; window.location.assign(<?= json_encode($whatsappBase) ?> + '?text=' + encodeURIComponent(message)); });
</script>
</body>
</html>
